...
async def make_query_chat_hist(dfRow, isZeroShot):
...
async def create_output_file(df, kernelName, targetName, isZeroShot, outfileName):
    subdf = df[(df['kernelName'] == kernelName) & (df['targetName'] == targetName)]
    
    assert subdf.shape[0] == 1

    for index, row in subdf.iterrows():
        messages = await make_query_chat_hist(row, isZeroShot)
...
async def main():
    ...
    parser.add_argument('--kernelName', type=str, default='mean_shift', help='Name of the kernel we want')
    ...
    await create_output_file(df, args.kernelName, args.targetName, args.zeroShot, args.outputFile)
